% ================================================================
% File: Superposition_Bateman_with_DioCache.txt
%
% Purpose:
...
%   using a precomputed Diophantine dictionary (DioCache).
%
% Main features:
...
%   5) External Diophantine cache loaded from a .mat file:
%         DioCache{p, m+1}
%   6) Garrappa-style generalized Mittag-Leffler evaluation:
%         ml(z, alpha, beta, gamma)
%
% Notes:
%   - Save this file as: Superposition_Bateman_with_DioCache.m
%   - Make sure Garrappa's ml(...) is on the MATLAB path.
%   - Build the file dio_cache_p14_m5.mat beforehand using the
%     companion routine build_dio_cache(...)
...
[Xn_series, history] = bateman_superposition_solver( ...
    lambda, x10, Time_vector, dt_block, Mmax, tol, alpha_eps, DioCache);
...
function [Xn_out, history] = bateman_superposition_solver( ...
    ...
    for b = 1:(numel(block_edges)-1)
        ...
        [state_req, state_end] = propagate_block_by_superposition( ...
            lambda, state0, tau_req, tau_end, Mmax, tol, alpha_eps, DioCache);
    ...
    for j = 1:n
        ...
        if ~isempty(tau_req)
            sub_req = local_subchain_solution( ...
                lambda_sub, xj0, tau_req, Mmax, tol, alpha_eps, DioCache);

            state_req(:, j:end) = state_req(:, j:end) + sub_req;
        end

        sub_end = local_subchain_solution( ...
            lambda_sub, xj0, tau_end, Mmax, tol, alpha_eps, DioCache);
...
function sub_state = local_subchain_solution( ...
    lambda_sub, xj0, tau, Mmax, tol, alpha_eps, DioCache)
    ...
    for r = 1:nr
        ...
        [xr, ~] = local_series_last_node( ...
            lambda_prefix, xj0, tau, Mmax, tol, alpha_eps, DioCache);
...
function [xn, info] = local_series_last_node( ...
    lambda, x10, t, Mmax, tol, alpha_eps, DioCache)
