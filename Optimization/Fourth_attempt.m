% ================================================================
% Optimized MATLAB implementation of a stepwise superposition solver
% for the last nuclide concentration X_n(t) in a linear decay chain.
%
% Main optimizations added with respect to the previous version:
%   1) The series metadata associated with each lambda-prefix is cached
%      (SeriesCache), so build_series_cache(...) is not recomputed.
%   2) Mittag-Leffler evaluations are cached at the VECTOR level instead
%      of the SCALAR level. This dramatically reduces the number of cache
%      key constructions and lookups.
%   3) Factorials up to Mmax are precomputed once.
%   4) Inner-loop progress printing was removed.
%
% Make sure:
...
filename = 'Bateman_superposition_results_optimized_v2.txt';
...
function [xn, info, MLCache, SeriesCache] = local_series_last_node( ...
    ...
    for ell = 1:numel(cache.levels)
        ...
        for q = 1:numel(level.coeff)
            ...
            [Eder, MLCache] = get_cached_ml_eval( ...
                z, alpha_eps, beta_eff, gamma_eff, fact_m, MLCache);

            Xm = Xm + coeff_q .* (t .^ gamma_q) .* Eder;
        end
...
function key = ml_vector_cache_key(z, alpha, beta, gamma)
    % Vector-level cache key. This reduces the number of key constructions
    % dramatically relative to the scalar-level cache.
    if isscalar(z)
        key = sprintf('S|%.16e|%.16e|%.16e|%.16e', z, alpha, beta, gamma);
    else
        key = sprintf('V|%.16e|%.16e|%.16e|%d|%.16e|%.16e|', ...
            alpha, beta, gamma, numel(z), z(1), z(end));
        % Safer fallback for nonuniform vectors
        if numel(z) > 2
            key = [key, sprintf('%.16e,', z(2:end-1))];
        end
    end
end
...
function cache = build_series_cache(lambda, Mmax, DioCache)
    ...
    % This is independent of x10 and can therefore be cached safely.
